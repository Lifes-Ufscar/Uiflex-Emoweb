(function () {
  ...
  var valen_camera = 0; // Inicializando valence como 0
  var arou_camera = 0; // Inicializando arousal como 0
  var tempo = 0;
  var json_obj = [];
  var json_array = [];
  ...
  var COLOR_MAP = 'images/color_map.jpg';
  ...
  var GRID_IMG = 'images/spectrum.png';
  ...
  var Overlays = function () {
    ...
    createClass(Overlays, [{
      ...
      value: function _initDomElements() {
        ...
        //"Escondendo" o circulo semantico
  ...
  var loader = CY.loader().licenseKey("f39756caa8caf4b09ce1a18e729ba128d82053f2233d").addModule(CY.modules().FACE_AROUSAL_VALENCE.name, { smoothness: 0.7 }).source(source);
  ...
  loader.load().then(function (_ref) {
    ...
    source.start().then(function () {
      ...
      window.addEventListener(CY.modules().FACE_AROUSAL_VALENCE.eventName, function (evt) {
        //console.log('Face arousal valence result', evt.detail.output.arousalvalence);
        valen_camera = evt.detail.output.arousalvalence.valence;
        arou_camera = evt.detail.output.arousalvalence.arousal;
  ...
  //Usar set interval para salvar no chrome
  setInterval(async function () {
    ...
    var angulo = (Math.atan2(valen_camera, -arou_camera) / (Math.PI / 180)) + 180
    /////////////Converter angulos para oitante(inicial_camera)
    if (angulo >= 0 && angulo < 45) {
      if(dataStorege.oitante_inicial_camera){
    ...
    if (angulo >= 135 && angulo < 180) {
      if(dataStorege.oitante_inicial_camera){
        ...
        chrome.storage.local.set({
          'oitante_camera': '4',
          'oitante_inicial_camera': '4'
        })
      }
    }
    if (angulo >= 180 && angulo < 225) {
      if(dataStorege.oitante_inicial_camera){
        chrome.storage.local.set({
          'oitante_camera': '5'
        })
    ...
    if (angulo >= 315 && angulo < 360) {
      if(dataStorege.oitante_inicial_camera){
        ...
        chrome.storage.local.set({
          'oitante_camera': '8',
          'oitante_inicial_camera': '8'
        })
      }
    }
    chrome.storage.local.set({
      'arousal_camera': arou_camera,
      'valence_camera': valen_camera,
      'tempo': tempo
    });

    var json = { "Tempo": tempo, "Valence": valen_camera, "Arousal": arou_camera };
    console.log('Valencia da camera:', valen_camera);
    console.log('Arousal da camera:', arou_camera);
    json_obj.push(json);
    json_array = JSON.stringify(json_obj);
    tempo = tempo + 1;
