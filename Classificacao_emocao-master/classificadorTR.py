    ...
    while True:
        try:
            ...
            if tipo == 'EEG':
                ...
                buffer_gsr_ecg[dados['id_gsr_ecg']] = {
            ...
            for id_gsr_ecg in list(buffer_gsr_ecg.keys()):
                # Encontra o bloco EEG correspondente
                bloco_encontrado = None
                for id_bloco, eeg_data in buffer_eeg.items():
                    # Verifica se o ID GSR/ECG está dentro do intervalo do bloco EEG
                    if id_bloco <= id_gsr_ecg < id_bloco + eeg_data['amostras_validas']:
                        bloco_encontrado = eeg_data
                        break
                ...
                if bloco_encontrado:
                    ...
                    if bloco_encontrado['resultado_eeg'] != -1:
                        with open('resultados_classificacao_sincronizados.txt', 'a') as file:
                            linha = (
                                ...
                                f"Amostras_EEG: {bloco_encontrado.get('amostras_validas', 20)}\n"
                            )
                            file.write(linha)
                    ...
                    # Remove o bloco EEG se todos os GSR/ECG correspondentes foram processados
                    ultimo_id_bloco = bloco_encontrado['id_bloco'] + bloco_encontrado['amostras_validas'] - 1
                    if not any(gsr_id <= ultimo_id_bloco for gsr_id in buffer_gsr_ecg.keys()):
                        del buffer_eeg[bloco_encontrado['id_bloco']]
...
def processar_eeg_sincronizado(arquivo_dados_brutos):
    ultima_linha_processada = 0
    batch_size = 20
    min_amostras = 16  # mínimo de dados para filtragem dos canais
    
    # Buffer para armazenar amostras pendentes
    buffer_amostras = []
    buffer_ids = []
    buffer_raw_data = []
    ...
    while True:
        ...
        # Se não há novos dados, verifica se temos algo no buffer para processar
        if dados_eeg is None or dados_eeg.shape[1] == 0:
            if len(buffer_amostras) >= min_amostras:
                # Processa o buffer se tiver amostras suficientes
                print(f"Processando buffer com {len(buffer_amostras)} amostras acumuladas")
                dados_eeg = np.array(buffer_amostras).T
                ids_brutos = buffer_ids
                raw_data = buffer_raw_data
                buffer_amostras = []
                buffer_ids = []
                buffer_raw_data = []
            else:
        ...
        # Se há novos dados, adiciona ao buffer
        if dados_eeg is not None and dados_eeg.shape[1] > 0:
            # Adiciona as novas amostras ao buffer
            for i in range(dados_eeg.shape[1]):
                buffer_amostras.append([dados_eeg[j][i] for j in range(dados_eeg.shape[0])])
            buffer_ids.extend(ids_brutos)
            buffer_raw_data.extend(raw_data)
            
            # Se o buffer atingiu o tamanho mínimo, processa
            if len(buffer_amostras) >= min_amostras:
                # Pega até batch_size amostras do buffer
                num_processar = min(len(buffer_amostras), batch_size)
                dados_processar = buffer_amostras[:num_processar]
                ids_processar = buffer_ids[:num_processar]
                raw_processar = buffer_raw_data[:num_processar]
                
                # Remove as amostras que serão processadas do buffer
                buffer_amostras = buffer_amostras[num_processar:]
                buffer_ids = buffer_ids[num_processar:]
                buffer_raw_data = buffer_raw_data[num_processar:]
                
                dados_eeg = np.array(dados_processar).T
                ids_brutos = ids_processar
                raw_data = raw_processar
            else:
                print(f"Aguardando mais amostras (buffer tem {len(buffer_amostras)}/{min_amostras})")
                time.sleep(0.5)
                continue
